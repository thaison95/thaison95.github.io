...
const writeDB = _.debounce(async (doc, data) => {
  ...
  if (+snapShotSize < 100 && +snapShotSize >= 10)
    snapShotSize = "0" + snapShotSize;
  await db
    .collection(clientID)
    .doc(snapShotSize + "_" + dateTime + "-" + doc)
    .set({ ...data });
  clickData = [];
...
function calculateTime() {
  ...
  clickData.push({
    ...
    speed: dayjs().second() + "." + dayjs().millisecond(),
  });
  ...
  while (i < 10) {
    ...
    if (timeToFeed.isBefore(dayjs().hour(23).minute(55))) {
      ...
      liEl += `
        <li>
          <button type="button" class="btn btn-danger btn-sm" style="margin-right: 10px" onclick="btnClick('minus', '${timeToFeed.format(
            "HH:mm"
          )}')">-</button>
            <span>${timeToFeed.format("HH:mm")}</span>
          <button type="button" class="btn btn-success btn-sm" style="margin-left: 10px" onclick="btnClick('plus', '${timeToFeed.format(
            "HH:mm"
          )}')">+</button>
        </li>
      `;
